...
export async function forwardToHub(request: Request, body?: string): Promise<Response|null> {
 ...
 try{
  ...
  const response=await fetch(target,{method:request.method,headers:request.method==='DELETE'?{'x-admin-token':request.headers.get('x-admin-token')??''}:body===undefined?undefined:{'Content-Type':'application/json'},body,cache:'no-store',redirect:'error',signal:AbortSignal.timeout(12000)});
  const contentType=response.headers.get('content-type')??'';
  if(!contentType.includes('application/json'))throw new Error('Upstream did not return JSON');
  if(request.method==='GET'&&response.ok)return NextResponse.json(normalizeState(await response.json()),{headers:{'Cache-Control':'no-store'}});
