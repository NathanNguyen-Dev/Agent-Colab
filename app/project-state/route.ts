...
export async function DELETE(request: Request) {
  ...
  // Relay only an explicitly addressed reset; upstream enforces its own token.
  if (projectId !== PROJECT_ID || url.searchParams.get("confirm") !== PROJECT_ID) {
    return NextResponse.json({ error: "project_id and confirm must both equal agent-colab" }, { status: 400, headers: noStore });
  }
  const forwarded = await forwardToHub(request);
  if (forwarded) return forwarded;
  ...
  try {
    ...
    return NextResponse.json(
      { error: "storage failure; reset outcome could not be confirmed" },
      { status: 503, headers: noStore },
    );
  }
